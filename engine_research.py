...
from engine_data import DataSet, load_csv_dataset, make_synthetic_dataset
...
def _sharpe_se(sr_per: float, n_obs: int, skew: float, kurt: float) -> float:
    """
    Approx Sharpe standard error (per-period SR) with non-normality adjustment:
      se ≈ sqrt((1 - skew*SR + ((kurt-1)/4)*SR^2) / (n-1))
    If skew/kurt are missing -> defaults: skew=0, kurt=3 (Normal).
    """
    n = int(n_obs)
    sr = float(sr_per)
    ...
    if (not np.isfinite(term)) or term <= 0.0:
        return float("nan")
    return float(np.sqrt(term / float(n - 1)))


def _deflated_sharpe_ratio(sr_per: float, n_obs: int, skew: float, kurt: float, trials: int, sr0_per: float = 0.0) -> float:
    """
    Deflated Sharpe Ratio as probability in [0,1].
    Uses Blom approximation for expected max among M trials:
      p = (M - 0.375) / (M + 0.25)
      z_M = Phi^{-1}(p)
      SR* = SR0 + se * z_M
...
def _pick_first(metrics: Dict[str, float], keys: List[str]) -> float:
    for k in keys:
...
def run_experiment(cfg: EngineConfig) -> int:
    ...
    # DSR trials = number of UNIQUE candidates tried in this experiment
    dsr_trials = max(1, int(n_candidates_unique))
    ...
    def _metric_value(metrics: Dict[str, float], key: str, fallback_n_obs: Optional[int]) -> float:
        """
        Read metric value for selection/scoring.
        Special case: dsr_net is computed VIRTUALLY from base metrics,
        because it depends on dsr_trials (experiment-specific) and must NOT be cached.
        """
        if key != "dsr_net":
        ...
        # ---- dsr_net virtual computation ----
        # Prefer per-period Sharpe if available; else derive from annualized sharpe_net.
        sr_per = _pick_first(metrics, ["sharpe_net_per", "sharpe_raw_net"])
        if not np.isfinite(sr_per):
            sharpe_ann = _pick_first(metrics, ["sharpe_net"])
            ppy = _pick_first(metrics, ["periods_per_year"])
            if not np.isfinite(ppy):
                ppy = 252.0
            if np.isfinite(sharpe_ann) and np.isfinite(ppy) and ppy > 0:
                sr_per = float(sharpe_ann / np.sqrt(ppy))

        n_obs_f = _pick_first(metrics, ["n_obs"])
        if np.isfinite(n_obs_f):
            n_obs = int(n_obs_f)
        else:
            n_obs = int(fallback_n_obs or 0)

        skew = _pick_first(metrics, ["skew_net", "skew"])
        kurt = _pick_first(metrics, ["kurt_net", "kurt"])
        ...
        return float(_deflated_sharpe_ratio(sr_per, n_obs, skew, kurt, trials=dsr_trials, sr0_per=0.0))
    ...
    for res in fold_results:
        ...
        train_n = int(res.get("train_n", 0) or 0)
        valid_n = int(res.get("valid_n", 0) or 0)
        ...
        # ---- selection metric values (DSR computed virtually here) ----
        v_val = _metric_value(m_valid, sel_metric, fallback_n_obs=valid_n) if valid.get("status") == "ok" else float("nan")
        t_val = _metric_value(m_train, sel_metric, fallback_n_obs=train_n) if train.get("status") == "ok" else float("nan")
        ...
        per_strategy_fold_compact[sh].append(
            {
                ...
                "train_n": train_n,
                "valid_n": valid_n,
                "train_metric": float(t_val),
                "valid_metric": float(v_val),
        ...
        if valid.get("status") == "ok" and np.isfinite(v_val):
            per_strategy_valid_vals[sh].append(float(v_val))
